...
def lookup_invite(Name):
        invite_lookup_query_SQL = f'SELECT * FROM `maggie-and-ollie-wedding.wedding_1805.invitations_table` WHERE Invite_Group_Name LIKE "%{Name}%"'
        invite_lookup_query_job = client.query(invite_lookup_query_SQL)
        invite_lookup_results = list(invite_lookup_query_job)

        invite_id = invite_lookup_results[0].Invite_ID
        invite_names_list = invite_lookup_results[0].Invite_Group_Name
        invitees_count = invite_lookup_results[0].Headcount
